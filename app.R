pkgs <- c("shiny", "shinyjs", "leaflet", "shinythemes", "leaflet.extras", "DT", "janitor", "ggplot2", "dplyr", "scales",
          "tidyverse", "lubridate", "RSocrata", "leaflet", "scales", "sf", "sp", "rgdal", "rsconnect")
...
# Calling shapefile
beats_shp = readOGR(dsn = "./ChicagoPoliceBeats",
                    layer = "chiBeats")
...
# Main ----------------------------------------------------------------------
ui = navbarPage(
    "Acidentes no Trânsito em Chicago",
    id = 'nav',
    ...
    tabPanel(
        ...
        sidebarPanel(
            ...
            sliderInput(
                ...
                value = c(min(crash$crash_date +
                                  1), min(crash$crash_date + 2))
                #min = mi,
                #max = ma,
                #value = va
            ),
...
server = function(input, output) {
    ...
    output$g1 = renderPlot({
        df() %>%
            ...
            labs(y = "% dos Acidentes",
                 x = NULL,
                 fill = "Acidente com machucados:") +
    ...
    output$t2 = renderText({
        if (input$type_m == 'Não houve feridos') {
            ...
            paste(
                "O dia com o maior número de feridos foi",
                strong(most[which.max(most$n), 1]),
                "com",
                strong(max(most$n)),
                "vítimas."
